<?php 
    ...
    foreach($rows as $row) {
		...
		$data .='<div class="healthymenuProductBox col-md-4 col-lg-3 col-xs-6 masonry-item '.$layout.'">
					<div class="next-lag">
						<div class="over_hot">
					...
					'</div>
					<div class="caption">
						<a href="javscript:{0}"  class="show_data" data-id="'.$row['product_id'].'">
							<h4>'.$row['name'].'</h4>
						</a>
						<p class="elips-content"><a href="javscript:{}" class="readmore '. ($strip ? 'catching': '').'" > '. strip_tags($row['content']) .' </a>'.($strip ? '<span class="display_all">Hiển thị thêm</span>': '').' </p>
					</div>
					<div class="price-addtocart">
						<div class="price-unit pull-left">
							<span class="productprice">'. number_format($row['price'], '0', '.','.' ) .'</span><span class="productunit"> VNĐ</span>
						</div>
						<input value="'. $row['product_id'] .'" type="hidden" name="id">
						<input type="hidden" name="addCart">
						<button type="button" data-children="'.$row['children'].'" class="btnAddtocart btnAddtocartText addtocart pull-right ' . $bhan . '" style="background: none;border:none;cursor: pointer;">
