 ...
 * @return bool
 */
function uploadVideo($url, $filename , $dir, $type = 0) {
	$test = false;
	if($type==0) $url = 'https://img.youtube.com/vi/'.$url.'/maxresdefault.jpg';
	elseif($type==1) $url = 'https://i.vimeocdn.com/video/'.$url.'.jpg';
	define("URL_1", $url);
...
function updateTags(array $tags) {
    global $db;
    $stringObj = new String();
    $result = array();
    if(count($tags)>0) {
        foreach($tags as $val) {
            $slug = $stringObj->getSlug($val);
            //---
            $db->table = "tags";
            $db->condition = "`name` LIKE '$val'"; //CONVERT('string', BINARY))
            $db->order = "";
            $db->limit = "";
            $rows = $db->select();
            if($db->RowCount>0) {
                ...
                $rows = $db->select();
                if($db->RowCount>0) {
                    $slug = $stringObj->getSlug($val . '-' . getRandomString(10));
                }

                $db->table = "tags";
                $data = array(
                    'name'=>$db->clearText($val),
...
function updateTags2(array $slug) {
    global $db;
    if(count($slug)>0) {
        foreach($slug as $val) {
            $val2 = '"' . $val . '"';
            $db->table = "article";
            $db->condition = "`tags_2` LIKE '%$val2%'";
            $db->order = "";
            $db->limit = "";
            $db->select();
            $count = $db->RowCount;
...
function extractListEmail($text) {
    $result = $list = array();
    $text = explode(',', $text);
    foreach ($text as $val) {
        array_push($list, trim($val));
    }
    $list = array_filter($list);
    $list = array_keys(array_flip($list));

    $list = implode(',', $list);
    preg_match_all("/[a-z0-9_\-\+\.]+@[a-z0-9\-]+\.([a-z]{2,3})(?:\.[a-z]{2})?/i", $list, $matches);

    foreach($matches[0] as $value) {
        $list = str_replace($value, '', $list);
    }
    ...
    if(count($list) > 0 && count($matches[0]) > 0 ) {
        $result['name'] = $list;
        $result['mail'] = $matches[0];
    }
