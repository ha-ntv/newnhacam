...
if($type=='updateInfo') {
	...
	if($db->RowCount>0) {
		...
		if($OK) {
            $full_name = trim(mb_convert_case($_POST['full_name'], MB_CASE_TITLE, "UTF-8"));
			...
			$data = array(
                ...
                'apply'=>$db->clearText(trim($apply)),
                'email'=>$db->clearText(trim($email)),
                'phone'=>$db->clearText(trim($phone)),
