...
$gallery_id = isset($_GET['id']) ? intval($_GET['id']) : intval($gallery_id);
$db->table = "gallery";
...
$rows = $db->select();
foreach($rows as $row) {
	$menu_id = intval($row['gallery_menu_id']);
}
...
if($typeFunc=='edit'){
	if(empty($name)) $error = '<span class="show-error">Vui lòng nhập tên hình.</span>';
		...
		if($OK) {
			...
			$db->table = "gallery";
			$data = array(
				'gallery_menu_id'=>intval($gallery_menu_id),
			...
			if($handleUploadImg) {
				...
				if($category_id==91) {
					...
					$imgUp->file_new_name_body    = 'home-' . $name_image;
					$imgUp->image_resize          = true;
					$imgUp->image_ratio_crop      = true;
                    $imgUp->image_x               = 370;
                    $imgUp->image_y               = 228;
